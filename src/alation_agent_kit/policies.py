...
class PolicyProvisioner:
    ...
    # -- rename ------------------------------------------------------------
    def _transition(self, rid: Any, current: str, target: str, token: str,
                    comment: str, log: list[str]) -> bool:
        """Walk a standard from `current` to `target`, reading back each hop.

        Extracted from publish() so rename can reuse it. Same rule: never assume
        a hop landed — the approval workflow can divert one, and continuing from
        an assumed state compounds the error.
        """
        hops = status_path(str(current), target)
        if hops is None:
            log.append(f"      ! no legal path {current} -> {target}")
            return False
        for hop in hops:
            try:
                self.c.post(f"{STANDARDS}{rid}/status/",
                ...
                body = getattr(err, "body", None)
                log.append(f"      ! hop -> {hop} refused: "
                           f"{json.dumps(body, default=str) if body is not None else err}")
                if getattr(err, "status", None) == 403:
                    log.append("        403 = role, not payload. Publishing needs "
                               "Global Standards Approver.")
                return False
            try:
                now = (self.c.get(f"{STANDARDS}{rid}/",
                                  extra_headers={"CDEToken": token})
                ...
                now = "unreadable"
            if str(now).upper() != hop:
                log.append(f"      ! asked for {hop}, instance reports {now!r} — "
                           f"the approval workflow intervened. Stopping.")
                return False
            log.append(f"      + {now}")
        return True

    def relink_standard(self, standard_id: Any, policy_id: int,
                        comment: str = "", dry_run: bool = True,
                        ) -> tuple[list[str], bool]:
        """Re-point a published standard's `sources` at a different business policy.

        WHAT THIS DOES NOT DO, and the caller must understand it: it changes the
        LABEL, not the DERIVATION. `derived_requirements` were generated by CDM
        from the ORIGINAL policy's prose and are carried forward untouched. After
        a relink the standard asserts a provenance it does not have.

        That matters here more than it would elsewhere, because the entire claim
        of this solution is that governance is traceable back to the regulation,
        and `sources[]` is the only thing carrying that claim. Use this to repair
        a development instance whose policies were churned. Do not use it to
        manufacture traceability on anything a customer will rely on — regenerate
        the standard from the live policy instead, which costs a permanent object
        but is true.

        Mechanically identical to rename_standard: new_version -> read in full ->
        PUT -> publish. `StandardUpdateRequest.sources` takes
        `{name, source_key, id}` and the published spec's own example re-points a
        standard to a different `alation://business_policy/N`, so this is a
        documented capability rather than a discovered one. It has NOT been
        executed here — the spec says what is forbidden, never what is allowed.
        """
        return self._revise_standard(
            standard_id, source_policy_id=policy_id, comment=comment,
            dry_run=dry_run)

    def rename_standard(self, standard_id: Any, new_name: str | None = None,
                        from_source_policy: bool = False, comment: str = "",
                        dry_run: bool = True) -> tuple[list[str], bool]:
        """Give a PUBLISHED standard a new name. See _revise_standard."""
        return self._revise_standard(
            standard_id, new_name=new_name, from_source_policy=from_source_policy,
            comment=comment, dry_run=dry_run)

    def _revise_standard(self, standard_id: Any, new_name: str | None = None,
                         from_source_policy: bool = False,
                         source_policy_id: int | None = None,
                         comment: str = "",
                         dry_run: bool = True) -> tuple[list[str], bool]:
        """Publish a new version of a standard with its name and/or sources changed.

        Give a PUBLISHED standard a new name, via a new version.

        A published standard cannot be edited and cannot be deleted — verified
        by execution 2026-08-28 and re-verified against the full schema
        2026-09-03 (PolicyStatus has exactly four values, none of them
        archived/retired; there are exactly eight Standard endpoints, none of
        them restore/revert; and `version` is a LIST FILTER, never an
        addressable parameter). So the ONLY way to change what a standard is
        called is to publish a new version carrying the new name. The old
        version remains in the version selector permanently. That is not a bug
        here, it is the shape of the API.

        Four steps per standard:
          1. POST /standard/{id}/new_version/   -> a DRAFT copy with a new id
          2. GET  /standard/{draft_id}/          -> read it back IN FULL
          3. PUT  /standard/{draft_id}/          -> same object, new name
          4. DRAFT -> PENDING_APPROVAL -> PUBLISHED

        Step 2 is not optional and is the reason this is code rather than four
        curl calls. `StandardUpdateRequest` requires `name`, `purpose`, `scope`
        AND the complete `derived_requirements` array with each requirement's
        and field's `id`. A PUT that omits them does not "leave them alone" —
        this is the same replace-not-merge trap as agent PATCH and workflow
        definitions. Hand-typing that array is how you silently drop a
        requirement from a governance standard.

        `from_source_policy` takes the name from `sources[0].name`, which is the
        policy CDM derived the standard from. That is the right name by
        construction and avoids six chances to typo.
        """
        log: list[str] = []
        try:
            token = self.cde_token()
        ...
        try:
            cur = self.c.get(f"{STANDARDS}{standard_id}/",
                             extra_headers={"CDEToken": token}) or {}
        except (AlationError, RuntimeError) as err:
            return ([f"  ! standard id={standard_id} unreadable: {str(err)[:200]}"],
                    False)

        old_name = cur.get("name") or ""
        status = str(cur.get("status") or "").upper()
        sources = cur.get("sources") or []

        if from_source_policy:
            if not sources or not (sources[0] or {}).get("name"):
                return ([f"  ! standard id={standard_id} {old_name!r} has no "
                         f"sources[].name to take a name from. Pass --name."], False)
            target = sources[0]["name"]
        elif new_name:
            target = new_name
        elif source_policy_id is not None:
            target = old_name          # relink only, name unchanged
        else:
            return (["  ! give --name, --from-source-policy or --source-policy"],
                    False)

        log.append(f"  standard id={standard_id} v{cur.get('version')} "
                   f"[{status}] {old_name!r}")
        log.append(f"      source policy: "
                   f"{(sources[0] or {}).get('name') if sources else '(none)'}"
                   f" ({(sources[0] or {}).get('source_key') if sources else '-'})")
        if target != old_name:
            log.append(f"      rename to: {target!r}")

        new_sources = None
        if source_policy_id is not None:
            # Resolve through the LIST, not GET /business_policies/{id}/ — the
            # list is proven working here and the by-id read is not. This is
            # also the guard that matters most in this command: relinking to a
            # policy that does not exist recreates the exact orphaning this is
            # meant to repair, so the policy must be READ, never assumed.
            try:
                pol = next((p for p in self._list_policies()
                            if str(p.get("id")) == str(source_policy_id)), None)
            except (AlationError, RuntimeError) as err:
                log.append(f"      ! could not list policies: {str(err)[:160]}")
                return (log, False)
            if pol is None:
                log.append(f"      ! policy id={source_policy_id} DOES NOT EXIST "
                           f"in Policy Center.")
                log.append("      ! refusing — relinking to a missing policy "
                           "recreates the orphaning this command exists to fix.")
                return (log, False)
            pol_title = pol.get("title") or pol.get("name") or ""
            if not pol_title:
                log.append(f"      ! policy id={source_policy_id} has no title: "
                           f"{json.dumps(pol, default=str)[:200]}")
                return (log, False)
            new_sources = [{"name": pol_title,
                            "source_key": f"alation://business_policy/{source_policy_id}"}]
            log.append(f"      relink to:  {pol_title!r} "
                       f"(alation://business_policy/{source_policy_id})")
            log.append("      NOTE: this changes the LABEL, not the derivation. "
                       "The requirements below were generated by CDM from the "
                       "PREVIOUS policy's prose and are carried forward unchanged.")

        if old_name == target and new_sources is None:
            log.append("      = already correct, nothing to do")
            return (log, True)
        if status != "PUBLISHED":
            # An unpublished standard can be PUT directly; no new version needed.
            log.append(f"      ! not PUBLISHED (is {status}). This command only "
                       f"handles published standards, because an unpublished one "
                       f"can be edited in place — and editing in place is "
                       f"reversible, so it should not go through here.")
            return (log, False)
        if dry_run:
            what = []
            if target != old_name:
                what.append("rename it")
            if new_sources is not None:
                what.append("re-point its source policy")
            log.append(f"      ~ would create a new version, "
                       f"{' and '.join(what)}, and publish")
            log.append("      ~ NOTE: the current version stays in the version "
                       "selector permanently. Versions are not deletable.")
            return (log, True)

        # 1. new version
        try:
            draft = self.c.post(f"{STANDARDS}{standard_id}/new_version/",
                                json_body={},
                                extra_headers={"CDEToken": token}) or {}
        except (AlationError, RuntimeError) as err:
            body = getattr(err, "body", None)
            log.append(f"      ! new_version refused: "
                       f"{json.dumps(body, default=str) if body is not None else err}")
            return (log, False)

        draft_id = draft.get("id") if isinstance(draft, dict) else None
        if draft_id is None:
            log.append(f"      ! new_version returned no id: "
                       f"{json.dumps(draft, default=str)[:300]}")
            return (log, False)
        log.append(f"      + draft version created, id={draft_id}")

        # 2. read it back IN FULL — the PUT replaces, so we must send everything
        try:
            full = self.c.get(f"{STANDARDS}{draft_id}/",
                              extra_headers={"CDEToken": token}) or {}
        except (AlationError, RuntimeError) as err:
            log.append(f"      ! draft id={draft_id} unreadable: {str(err)[:200]}")
            log.append(f"      ! a DRAFT version now exists and is not renamed. "
                       f"It is deletable: DELETE {STANDARDS}{draft_id}/")
            return (log, False)

        reqs = full.get("derived_requirements") or []
        body = {
            "name": target,
        ...
        if new_sources is not None:
            body["sources"] = new_sources
        elif full.get("sources"):
            body["sources"] = full["sources"]

        log.append(f"      carrying forward {len(reqs)} requirement(s) unchanged")

        # 3. PUT the whole object back with only the name changed
        try:
            self.c.put(f"{STANDARDS}{draft_id}/", json_body=body,
                       extra_headers={"CDEToken": token})
        except (AlationError, RuntimeError) as err:
            eb = getattr(err, "body", None)
            log.append(f"      ! update refused: "
                       f"{json.dumps(eb, default=str) if eb is not None else err}")
            log.append(f"      ! DRAFT id={draft_id} exists un-renamed and is "
                       f"deletable: DELETE {STANDARDS}{draft_id}/")
            return (log, False)
        log.append(f"      + renamed draft id={draft_id}")

        # 4. publish it
        if not self._transition(draft_id, "DRAFT", "PUBLISHED", token, comment, log):
            log.append(f"      ! draft id={draft_id} is renamed but NOT published, "
                       f"so the old name is still what displays. It is still "
                       f"deletable while DRAFT or PENDING_APPROVAL.")
            return (log, False)

        # verify by reading the current published version back
        try:
            now = self.c.get(f"{STANDARDS}{draft_id}/",
                             extra_headers={"CDEToken": token}) or {}
            log.append(f"  + id={draft_id} v{now.get('version')} "
                       f"[{now.get('status')}] {now.get('name')!r}")
        except (AlationError, RuntimeError):
            log.append("      ! could not read back — check the UI")
        return (log, True)
